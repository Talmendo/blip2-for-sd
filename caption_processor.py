...
class CaptionProcessor:
  ...
  def sanitise_prompt_shard(self, prompt):
    return prompt.split("Answer:")[0].strip().replace(", a point and shoot camera", "").replace(", a point and shoot", "").replace("it is a ", "").replace("it is ", "").replace("wearing nothing", "nude")
  ...
  def caption_me(self, initial_prompt, image):
    ...
    try:
      ...
      p_action = self.ask("What is she doing? Could be something like standing, stretching, walking, etc", image)
      # print(p_action)
      ...
      p_lighting = self.ask("What is the lighting like? Use professional terms, like: soft lighting, studio lighting, natural lighting and so on", image)
      # print(p_lighting)
